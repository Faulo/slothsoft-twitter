...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/2000/svg"
	...
	<xsl:template match="monthList" mode="svg">
		...
		<svg contentScriptType="application/javascript" contentStyleType="text/css" color-rendering="optimizeSpeed"
			shape-rendering="optimizeSpeed" text-rendering="optimizeSpeed" image-rendering="optimizeSpeed" width="{$width}px"
			height="{$height}px" class="log-short">
			<g class="online" transform="translate(0, {$height}) scale(1, -1)">
				<xsl:for-each select="month">
					<xsl:sort select="@time" />
					<g transform="translate({$scaleX * (position() - 1)}, 0)">
						<xsl:for-each select="user">
							<rect x="0" y="{$scaleY * @offset}" width="{$scaleX}" height="{$scaleY * @count}" data-user="{@name}"
								title="{@name}: {round(@count)} tweets" />
						</xsl:for-each>
						<text transform="translate({$scaleX div 2}, 26) rotate(90) scale(1, -1)">
							<xsl:value-of select="@name" />
							<!-- <xsl:text>: </xsl:text> <xsl:value-of select="@count"/> <xsl:text> tweets</xsl:text> -->
						</text>
					</g>
				</xsl:for-each>
			</g>
